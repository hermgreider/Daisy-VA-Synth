...
// midi handler
void HandleMidiMessageOld(MidiEvent m)
{
    switch(m.type)
    {
        case NoteOn:
        {
            ...
            // hardware.PrintLine("NoteOn, chn %d note %d", p.channel, p.note);
            // hardware.PrintLine("NoteOn, seqmode %d", seqmode);
            // snprintf(tx, 100, "NoteOn, chn %d note %d\n", p.channel, p.note);
        ...
        case NoteOff:
        {
            ...
            // hardware.PrintLine("NoteOf, chn %d note %d", p.channel, p.note);
            snprintf(tx, 100, "NoteOff, chn %d note %d\n", p.channel, p.note);
            uart.BlockingTransmit((uint8_t *)tx, strlen(tx), 1000);
